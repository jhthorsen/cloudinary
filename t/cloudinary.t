...
my $cloudinary = Cloudinary->new({ api_key => '1234567890', api_secret => 'abcd', cloud_name => 'demo', });
...

{
  ...
  for my $file ({ file => $0, filename => 'cloudinary.t' }, Mojo::Asset::File->new(path => $0),) {
      ...
      start => sub {
        ...
        for my $part (@{ $tx->req->content->parts }) {
          if(ref($part->asset) eq 'Mojo::Asset::File') {
            ...
            is($part->headers->content_disposition, 'form-data; name="file"; filename="cloudinary.t"',
              '...filename=$0');
          }
        }
      }
    ...
    # returns an id. need to use once(start) above to run tests
    $cloudinary->upload(
      { file => $file, timestamp => 1315060510, public_id => 'sample', on_success => sub { }, on_error => sub { }, });
  }
}

{
  $cloudinary->_ua->once(
    start => sub {
      ...
      is($tx->req->param('timestamp'), 1315060510,                                          '...with timestamp');
      is($tx->req->param('public_id'), 'sample',                                            '...with public_id');
      is($tx->req->param('signature'), '9c549a22def9e2690384973d77b3ff79d7b734d7',          '...with signed request')
        ;    # different key because of "type" is included in POST
    }
  );

  # returns an id. need to use once(start) above to run tests
  $cloudinary->destroy({ timestamp => 1315060510, public_id => 'sample', on_success => sub { }, on_error => sub { }, });
}

{
  is($cloudinary->url_for('sample.gif'), 'http://res.cloudinary.com/demo/image/upload/sample.gif',
    'url for sample.gif');
  is(
